<?php
	...
	$template = str_replace("{@menu-title@}","Unit Distribution",$template);
	$template = str_replace("{@menu-description@}",$_SESSION["ws-name"],$template);
	$template = str_replace("{@module@}","apps",$template);
	$template = str_replace("{@sub-module@}","unit distribution",$template);
	...
	$template = str_replace("{@outstanding-po-content@}",$outstanding_do,$template);
